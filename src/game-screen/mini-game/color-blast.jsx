...
export default function Cblast({
	onChangeWorld,
	startPosition,
	character = "ucup2",
	username = "Player",
	...
	const createExplosion = (x, y) => {
		...
		for (let i = 0; i < numParticles; i++) {
			particles.push({
				...
				size: random(4, 12),
				color: ["#ff6b35", "#f7931e", "#ffcc02", "#fff200", "#ff4757"][
					random(0, 5)
				],
				life: 1.0,
			...
			return prevExplosions
				.map((explosion) => {
					...
					if (
						...
						currentTime - explosion.startTime > EXPLOSION_DURATION
					) {
						return null;
					}
	...
	useEffect(() => {
		...
		const cameraTransformLeft =
			-gameState.cameraX * PIXEL_SIZE + PIXEL_SIZE * CAMERA_LEFT_OFFSET_PX;
		const cameraTransformTop =
			-gameState.cameraY * PIXEL_SIZE + PIXEL_SIZE * CAMERA_TOP_OFFSET_PX;
		...
		characterRef.current.style.transform = `translate3d(${
			gameState.x * PIXEL_SIZE
		}px, ${gameState.y * PIXEL_SIZE}px, 0)`;
			...
			{showGameOverAlert && (
				<div
					...
					<div
						...
						<p
							style={{ fontSize: "18px", marginBottom: "30px", color: "#ccc" }}>
							{enemies.length > 0
    ...
    `}
			...
			<div
				ref={mapRef}
				className="map"
				style={{ backgroundImage: `url(${beachMape})` }}>
				...
				{enemies.map((enemy) => (
					<div
						...
						style={{
							...
							transform: `translate3d(${enemy.x * PIXEL_SIZE}px, ${
								enemy.y * PIXEL_SIZE
							}px, 0)`,
							position: "absolute",
						}}
					...
					<div key={explosion.id} style={{ position: "absolute" }}>
						{explosion.particles.map((particle) => (
							<div
								...
								style={{
									...
									borderRadius: "50%",
									transform: `translate3d(${particle.x * PIXEL_SIZE}px, ${
						...
						<div
							style={{
								...
								borderRadius: "50%",
								background:
									"radial-gradient(circle, rgba(255,255,255,0.8) 0%, rgba(255,107,53,0.6) 30%, rgba(255,71,87,0.4) 60%, transparent 100%)",
								transform: `translate3d(${
									(explosion.x + 16) * PIXEL_SIZE - 30
								}px, ${(explosion.y + 16) * PIXEL_SIZE - 30}px, 0)`,
				...
				{bullets.map((bullet) => (
					<div
						...
						style={{
							...
							transform: `translate3d(${bullet.x * PIXEL_SIZE}px, ${
								bullet.y * PIXEL_SIZE
							}px, 0)`,
						}}
					/>
