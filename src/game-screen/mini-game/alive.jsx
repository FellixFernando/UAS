...
export default function Alive({
	...
	const accessibleCellsSet = useMemo(
		() => calculateAccessibleCells(gameMap, PLAYER_INITIAL_SPAWN, GRID_SIZE),
		[gameMap]
	);
	...
	const VIEWPORT_WIDTH_CELLS = Math.floor(
		(viewportDimensions.width * 1.027) / CELL_SIZE
	);
	const VIEWPORT_HEIGHT_CELLS = Math.floor(
		(viewportDimensions.height * 1) / CELL_SIZE
	);
	...
	const getRandomEmptyPosition = useCallback(() => {
		...
		do {
			posKey =
				accessibleArray[Math.floor(Math.random() * accessibleArray.length)];
	...
	const handleKeyDown = useCallback((e) => {
		if (KEYS[e.code])
			setPressedKeys((prev) => new Set([...prev, KEYS[e.code]]));
	}, []);
	...
	useEffect(() => {
		...
		const pathfindingInterval = setInterval(() => {
			...
			setNpcs((currentNpcs) => {
				if (currentNpcs.length < NPC_COUNT) {
					console.warn(
						`NPC count (${currentNpcs.length}) is less than expected (${NPC_COUNT}). Re-initializing NPCs.`
					);
					const newNpcs = Array.from({ length: NPC_COUNT }, (_, i) => {
						const existingNpc = currentNpcs.find(
							(npc) => npc.id === `npc-${i}`
						);
	...
	useEffect(() => {
		...
		for (const npc of npcs) {
			...
			if (playerGridX === npcGridX && playerGridY === npcGridY) {
				...
				setNpcs((npcs) =>
					npcs.map((n) => ({ ...n, walking: false, path: [] }))
				);
				return;
			}
		}
	...
	// --- CAMERA POSITION ---
	const camX = Math.max(
		Math.min(VIEWPORT_WIDTH_PIXELS / 2 - playerPos.x * CELL_SIZE, 0),
		-(GRID_SIZE * CELL_SIZE - VIEWPORT_WIDTH_PIXELS)
	);
	const camY = Math.max(
		Math.min(VIEWPORT_HEIGHT_PIXELS / 2 - playerPos.y * CELL_SIZE, 0),
